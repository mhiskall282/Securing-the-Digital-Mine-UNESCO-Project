<div wire:poll.2s="autoSniffTick" class="space-y-8">
    ...
    <div class="bg-[#101726] border border-white/5 p-6 rounded-3xl shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            <button wire:click="toggleStreaming" class="px-4 py-2 rounded-xl text-xs font-semibold transition cursor-pointer flex items-center gap-2 border @if($isAutoSniffing) bg-emerald-500/10 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/20 @else bg-amber-500/10 text-amber-400 border-amber-500/30 hover:bg-amber-500/20 @endif">
                ...
                {{ $isAutoSniffing ? 'Streaming Active (Pause)' : 'Streaming Paused (Resume)' }}
            </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-[#101726] border border-white/5 p-5 rounded-2xl">
            ...
            <div class="mt-2 text-3xl font-extrabold text-white">{{ number_format($totalFlows) }}</div>
        ...
        <div class="bg-[#101726] border border-white/5 p-5 rounded-2xl">
            ...
            <div class="mt-2 text-3xl font-extrabold text-emerald-400">{{ number_format($normalCount) }}</div>
        ...
        <div class="bg-[#101726] border border-white/5 p-5 rounded-2xl">
            ...
            <div class="mt-2 text-3xl font-extrabold text-red-400">{{ number_format($attackCount) }}</div>
        ...
        <div class="bg-[#101726] border border-white/5 p-5 rounded-2xl relative overflow-hidden">
            ...
            <div class="mt-2 text-3xl font-extrabold @if($anomalyRate > 0) text-red-400 @else text-emerald-400 @endif">{{ $anomalyRate }}%</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="bg-[#101726] border border-white/5 p-6 rounded-3xl shadow-lg lg:col-span-4 space-y-6">
            ...
            <div class="space-y-4">
                @foreach($classDistribution as $label => $count)
                    @php
                    ...
                    <div class="space-y-1">
                        <div class="flex justify-between text-xs">
                            <span class="font-mono text-gray-300">{{ $label }}</span>
                            <span class="text-gray-400 font-mono">{{ $count }} ({{ round($percentage) }}%)</span>
                        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-white/5 text-xs text-gray-300 font-mono">
                        @forelse($latestFlows as $flow)
                            @php
                                $isAnomaly = $flow->prediction !== 'Normal';
                            @endphp
                            <tr class="hover:bg-white/[0.01]">
                                <td class="py-2.5 text-gray-500">{{ date('H:i:s', strtotime($flow->timestamp)) }}</td>
                                <td class="py-2.5">
                                    <span class="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-white/5 text-cyan-400">{{ strtoupper($flow->protocol_type) }}</span>
                                </td>
                                <td class="py-2.5 text-gray-400">{{ $flow->service }}</td>
                                <td class="py-2.5 text-gray-400">{{ $flow->flag }}</td>
                                <td class="py-2.5 text-right">{{ number_format($flow->src_bytes) }} B</td>
                                <td class="py-2.5 text-center">
                                    ...
                                    @else
                                        <span class="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-red-500/10 text-red-400 border border-red-500/25">{{ $flow->prediction }}</span>
                                    @endif
                                </td>
                                <td class="py-2.5 text-right font-semibold @if($isAnomaly) text-red-300 @else text-emerald-300 @endif">{{ number_format($flow->confidence, 2) }}%</td>
                                <td class="py-2.5 text-right pr-2 text-gray-400">{{ number_format($flow->latency_ms, 2) }} ms</td>
                            </tr>
                        @empty
                            <tr>
